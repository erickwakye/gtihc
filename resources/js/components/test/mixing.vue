                    ...
                    <tbody>
                        <tr v-for="test in tests.data" :key="test.id">
                            ...
                            <td>
                                ...
                                <button class="btn btn-sm btn-warning" @click="editTest(test)" > <i class="fa fa-edit"></i> Take Mixing</button>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="createTestModalLabel" v-show="!editMode">Mix</h5>
                        <h5 class="modal-title" id="createTestModalLabel" v-show="editMode">Mix</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form @submit.prevent="!editMode ? createPayment() : updateMix()">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label> Mixing Status </label>
                                <b-form-select v-model="form.mixed_status" text-field="mixed_status" class="mb-3" >
                                <b-form-select-option :value="null">Select Status</b-form-select-option>
                                <b-form-select-option value="1">Mixed</b-form-select-option>
                                <b-form-select-option value="0">Not Mixed</b-form-select-option>
                                </b-form-select>
                                <!-- <div class="mt-2">Selected: <strong>{{ selected }}</strong></div> -->
                                <has-error :form="form" field="mixed_status"></has-error>
                            </div>
                        ...
                        <div class="modal-footer justify-content-between">
                            ...
                            <button type="submit" v-if="load" v-show="editMode" class="btn btn-lg btn-success">Submit Mix</button>
                        </div>
    ...
    methods:{
        ...
        gotoURL(url){
         ...
         search(){
            ...
            this.getMixes();
            this.loading = true;
            axios.get(`/getAllMixes?s=${this.searchWord}`).then((response) =>{
                this.loading = false;
                this.isSearch = true;
        ...
        getMixes(){
           this.page=this.pagination.page;
            ...
            axios.get('/getAllMixes',this.pagination).then((response) =>{
               this.loading = false;
                this.tests = response.data.tests;
        ...
        updateMix(){
            this.action = 'Update Mix Testing ...'
            this.load = false;
            this.form.put("/mix/test/update/" +this.form.id).then((response) => {
                this.load = true;
                this.$toastr.s("Mix Test Taken successfully", "Created");
                Fire.$emit("loadTest");
                $("#createTest").modal("hide");
                this.form.reset();
            }).catch(() => {
                this.load = true;
                this.$toastr.e("Cannot Mix Test Taken, try again", "Error");
            });
        },
